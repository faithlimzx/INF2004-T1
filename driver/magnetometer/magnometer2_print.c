...
int main() {
    ...
    while (1) {
        ...
        i2c_write_blocking(i2c0, accelero_addr, config, 2, false);
        i2c_write_blocking(i2c0, accelero_addr, config_accel, 2, false);
        sleep_ms(10); // Wait for accelerometer data

        // Read accelerometer data
        uint8_t reg_accel = 0x28; // X-axis lsb data register
        i2c_write_blocking(i2c0, accelero_addr, &reg_accel, 1, true);

        uint8_t accel_data[6] = {0};
        i2c_read_blocking(i2c0, accelero_addr, accel_data, 6, false);
        ...
        i2c_write_blocking(i2c0, magneto_addr, config_magneto, 2, false);
        i2c_write_blocking(i2c0, magneto_addr, config_cra, 2, false);
        i2c_write_blocking(i2c0, magneto_addr, config_crb, 2, false);
        sleep_ms(10); // Wait for magnetometer data

        // Read magnetometer data
        uint8_t reg_magneto = 0x03; // X-axis msb data register
        i2c_write_blocking(i2c0, magneto_addr, &reg_magneto, 1, true);

        uint8_t magneto_data[6] = {0};
        i2c_read_blocking(i2c0, magneto_addr, magneto_data, 6, false);
